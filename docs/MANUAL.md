...
- **На острове:** пока играет, сверху висит пилюля с обложкой, названием и эквалайзером в цвет обложки. После паузы она исчезает через несколько секунд. Клик открывает плеер: перемотка, очередь («Далее: …»), кнопка «Показать в папке». Та же музыка видна в меню острова.
