...
JustDay — голосовой ИИ-агент для Linux-компьютера с KDE Plasma 6 (Wayland). Нажимаешь кнопку, говоришь, и он делает: открывает приложения и сайты, управляет окнами, мышью и клавиатурой, ищет файлы, запускает игры, читает почту, раздаёт задачи Claude Code и проверяет их работу.

Название: в «Железном человеке» после JARVIS появилась FRIDAY; здесь — JustDay (по аналогии с проектом Just Install It). Голосовое имя ассистента настраивается отдельно (`user.assistant_name`, по умолчанию «Джарвис»).
...
2. **Запись.** Демон слушает микрофон через PipeWire. Детектор речи Silero VAD понимает, когда вы замолчали (1 с тишины). Если кнопку зажать, запись идёт, пока держите.
3. **Распознавание.** Whisper large-v3-turbo на видеокарте переводит речь в текст примерно за 0,5 с. Звук никуда не отправляется.
4. **Маршрутизация** (`daemon.handle_utterance`):
   - ждём подтверждения опасного действия → «да» или «нет»;
   - «стоп», «отмена» → отменить всё;
   - **мгновенный путь** (`fastpath.py`) → «открой/закрой/сверни X», «громче», «пауза», «громкость 50», «заблокируй экран»; выполняется за 0,1–1 с без ИИ;
...
5. **Мозг.** Постоянная сессия Claude Code (`brain.py`) с персоной `brain/PERSONA.md`, профилем `CLAUDE.md`, навыками из `plugin/skills` и MCP-серверами. Модель сама решает, какие инструменты вызвать.
6. **Ответ.** Вслух произносится только итоговый текст хода. Промежуточные мысли показываются на Dynamic Island. Успешные действия без вопроса проходят молча, с коротким звуком.
7. **Фоновые задачи.** Работу с кодом мозг поручает отдельным сессиям Claude Code (`justday claude start`). Демон каждые 15 с проверяет их состояние и, когда сессия закончила или ждёт вас, отдаёт мозгу событие. Мозг сам смотрит diff и тесты, при необходимости «подпинывает» Claude и докладывает голосом.
...
- **GUI** через [kwin-mcp](https://github.com/VibeProgramm/kwin-mcp): ввод идёт через KWin EIS, дерево доступности через AT-SPI. Поверх него JustDay добавляет два инструмента (`plugin/bin/kwin_live.py`):
  ...
  Модель не пересчитывает координаты и не тратит отдельный ход на каждый клик.
...
2. Ставит недостающие системные пакеты через sudo: `pipewire-utils`, `wl-clipboard`, `playerctl`, `yt-dlp`, `plocate`, `fd`, `ripgrep`, `jq`, `spectacle`, `libnotify`, `espeak-ng`, `kitty`, `ImageMagick`, `zstd`, `libsecret`, `gtk4-layer-shell`, `python3-gobject`, `python3-cairo`.
...
8. Регистрирует горячие клавиши KDE: Meta+J и F19 для разговора, Meta+Shift+J для отмены.
9. При первой установке запускает **мастер настройки** `justday setup` (7 шагов, Enter оставляет значение по умолчанию):
   1. имя ассистента и как обращаться к вам;
   2. модель: Claude (откроет вход, если нужно), локальная (сама поставит Ollama + Qwen), OpenRouter или DeepSeek (попросит ключ);
   3. голос: нейросетевой (сам поставит Qwen3-TTS, ~4 ГБ) или простой Silero;
   4. микрофон;
   5. кнопки;
   6. почта (по желанию);
   7. город для погоды на острове.
...
3. **Почта** (по желанию): `justday mail setup` ([раздел 7](#7-почта)).
...
**Наведение на верхний край** показывает время, дату, последний ответ и погоду (город задаётся в настройках).

**Клик по острову** раскрывает меню: поле «Спросите или попросите…» (текстом, как голосом), кнопка микрофона, переключатели (звуки, уведомления, объявление писем, метки кнопок), выбор модели (Claude / Локальная / OpenRouter / DeepSeek с кнопкой «Перезапустить»), плеер (MPRIS), недавние запросы, «Новый разговор», «Остановить», «Журнал», «Руководство». Закрыть: Esc, крестик или клик мимо.

**Настройки** (шестерёнка в меню) открываются прямо в острове: боковое меню и разделы с анимацией.
- **Общие:** имена, обращение, автозапуск, язык.
- **Остров и анимации:** пружинные, плавные или выключены; появление при наведении; монитор; погода.
- **Голос и звук:** нейросетевой голос или Silero, выбор и прослушивание голосов, создание голоса по описанию, клонирование из записи, микрофон и вывод, чувствительность.
- **Кнопки:** сочетания и кнопка мыши.
- **Модель:** провайдер, модель, ключ, «сколько думать».
- **Почта:** подключение и поведение.
- **Люди:** записная книжка.
- **Память:** просмотр, редактирование и удаление заметок, новая заметка, профиль.
- **Приватность:** что уходит в облако, очистка журнала.
- **Диагностика:** службы, проверка, журнал.
- **О программе.**
...
- **Нейросетевой** ([Qwen3-TTS](https://github.com/QwenLM/Qwen3-TTS), Apache 2.0, ускорение [faster-qwen3-tts](https://github.com/andimarafioti/faster-qwen3-tts)).
  ...
  - Начинает говорить примерно через 0,4 с, генерирует в 2 раза быстрее реального времени.
  - Если служба недоступна, JustDay сам переходит на Silero.
...
- **Разрешения Claude Code** (`brain/settings.json`):
  - `allow`: безопасные частые действия идут без проверки, это ускоряет работу. Сюда входят `look`, список окон, `justday apps/windows/games`, `xdg-open`, `playerctl`, громкость и т. п. Команды, через которые можно выполнить произвольный код (`kitty`, `yt-dlp --exec`, `fd -x`), в список не входят.
  - `ask`: `rm -rf`, `sudo`, `dd`, `mkfs`, force-push, `git reset --hard`, удаление пакетов, файрвол, загрузчик, `systemctl disable`, публикации. JustDay спрашивает голосом и уведомлением с кнопками.
...
| Секция | Ключи |
|---|---|
| `[user]` | `name` (подпись в письмах), `address_as` («сэр»), `assistant_name` («Джарвис») |
...
| `[tts]` | `engine` (silero/espeak/none), `speaker` (aidar, eugene, baya, kseniya, xenia) |
...
### Своё (~4600 строк Python, ~2200 строк QML + навыки в markdown)
| Файл | Что делает |
|---|---|
...
| `plugin/skills/*/SKILL.md` | рецепты: desktop, discord, browser, claude-code, files, games, email, kindle |
...
- **Характер:** `brain/PERSONA.md`, затем `justday new-session`.
- **Голос:** `[tts] speaker`.
- **Другое имя:** `[user] assistant_name`; для лучшего распознавания добавьте имя в `[stt] initial_prompt`.
...
- **Настройка под голос пользователя** (как у Siri): чтение контрольных фраз.
  - Отпечаток голоса, чтобы откликаться только на вас.
  - Дообучение openWakeWord на ваших записях.
  - Подстройка паузы конца фразы под ваш темп.
- **Управление в реальном времени** для игр («пройти паркур в Roblox»): удержание клавиш на время, поворот камеры относительным движением мыши, быстрый цикл «кадр → действие» на локальной модели зрения.
- **Системные уведомления на острове** (сейчас показываются события самого JustDay).
...
```bash
git -C ~/JustDay pull && ~/JustDay/install.sh     # обновление JustDay (и зависимостей)
