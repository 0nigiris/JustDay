...
- **«Какую именно?»** Когда просьба подходит сразу нескольким записям — малоизвестный исполнитель, одинаковые названия, своё в фонотеке против находки на YouTube, — остров показывает до трёх вариантов с обложками, каналом и длительностью, и ждёт ответа. Отвечают нажатием, голосом («вторую», «ту, что с клипом», названием) или <kbd>Meta</kbd>+<kbd>Y</kbd> — это первый вариант. Молчание две минуты, выключенный голос и очевидное попадание («Flower Man» — он один) вопроса не вызывают: играет лучшее совпадение, как и раньше.
...
| `[brain]` | `provider`, `model`, `base_url`, `context_tokens`, `effort` (low/medium/high), `permission_mode`, `chrome`, `resume_within_hours`, `tool_search` (on/auto/off — схемы инструментов по требованию), `context_window` (потолок разговора в токенах, дальше история сжимается) |
...
- **Резервные копии перед правками.** `scripts/snapshot.sh` складывает рабочее дерево в
  `~/.local/share/justday/backups` — только то, что видит git, поэтому копия весит мегабайты и
  снимается за десятую долю секунды. Хук `PreToolUse` в `.claude/settings.json` зовёт её перед
  каждой правкой Claude Code; когда с прошлого раза ничего не менялось, копия не создаётся.
  ```bash
  scripts/snapshot.sh save "перед переделкой плеера"  # снять вручную
  scripts/snapshot.sh list                            # что накопилось
  scripts/snapshot.sh back                            # вернуть последнюю (перед откатом снимет ещё одну)
  scripts/snapshot.sh back 20260924-134754-проверка   # вернуть конкретную
  scripts/snapshot.sh drop --accepted                 # правки приняты: оставить только последнюю
  ```
  Хранятся последние 12 (`JUSTDAY_BACKUPS_KEEP`), каталог задаётся `JUSTDAY_BACKUPS`.
...
- схемы инструментов подгружаются по мере надобности (первый вызов нового инструмента — лишний шаг, зато каждый шаг втрое дешевле);
...
### Во что обходится разговор

Платится не длина ответа, а контекст: **каждый шаг перечитывает разговор целиком**. Один ход — это обычно 3–5 шагов, значит контекст оплачивается 3–5 раз за просьбу. Отсюда два правила: держать контекст коротким и не делать лишних шагов.

```bash
justday tokens              # по дням: шагов, контекст на шаг, деньги
justday tokens --days 30
```
...
| Мера | Было | Стало |
|---|---|---|
| Схемы инструментов (`brain.tool_search = "on"`) | 111 000 токенов на старте | 31 000 |
| Потолок разговора (`brain.context_window = 200000`) | рос до миллиона, а после 200 000 — вдвое дороже по тарифу | на потолке история сжимается сама |
| Читающие команды в `allow` | классификатор проверял каждую | правило отвечает без запроса к модели |
| «Начни заново», «сбрось контекст», «новая тема» | — | разговор с чистого листа, без обращения к модели |

Скажите «начни заново», когда тема сменилась: длинный разговор о домашке дорожает каждый шаг, а новый начинается с 31 тысячи токенов. Чтобы вернуть загрузку всех схем сразу (быстрее на первом вызове инструмента, дороже на каждом шаге), поставьте `tool_search = "off"`; `"auto"` оставляет решение Claude Code.
