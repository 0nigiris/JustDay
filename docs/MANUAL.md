...
flowchart TD
    ...
    F --> TTS
    M --> TTS
    B --> TTS["Голос: Silero TTS (локально) → pw-play"]
    D --> O["Индикатор-«остров» сверху экрана"]
```
...
6. **Ответ.** Вслух произносится только итоговый текст хода. Промежуточные мысли показываются на индикаторе. Успешные действия без вопроса проходят молча, с коротким звуком.
