...
class ActionKind(StrEnum):
    ...
    MACRO = "macro"
    """Несколько уже описанных действий подряд, одним нажатием."""
...
class ActionDescriptor(BaseModel):
    ...
    steps: list[ActionId] = Field(
        default_factory=list,
        description=(
            "Для мультидействия — идентификаторы шагов по порядку. "
            "Интерфейс показывает их на кнопке, чтобы было видно, что "
    ...
    session: str | None = Field(
        None,
        description=(
            "Имя tmux-сессии для действий вида tmux. Интерфейс по нему "
            "предлагает открыть терминал прямо в той сессии, которую "
            "кнопка запустила, — иначе она выглядит неработающей."
        ),
    )
