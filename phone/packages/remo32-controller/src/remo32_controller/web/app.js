...
/* Телефон, положенный набок, — другой прибор: стримдек. Решает не настройка,
   ...
   отделяет повёрнутый телефон (там высота экрана — это его ширина, 360–480)
   от планшета и ноутбука, где «боком» — обычное положение, а не жест. */
const DECK_MEDIA = matchMedia("(orientation: landscape) and (max-height: 500px)");
