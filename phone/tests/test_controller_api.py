...
class TestСтатикаИКэш:
    ...
    def test_крупный_и_малый_заголовок_оба_на_месте(self, anon: TestClient) -> None:
        """Заголовок экрана живёт в разметке дважды: крупный под полосой и
        маленький внутри неё — второй появляется, когда первый уезжает при
        прокрутке. Оба ведёт один и тот же код, и если один потерять,
        заголовок просто перестанет меняться, ничего не сломав заметно."""
        html = anon.get("/").text
        assert 'id="page-title"' in html
        assert 'id="page-title-sm"' in html

    def test_стримдек_описан_в_стилях(self, anon: TestClient) -> None:
        """Повёрнутый телефон — отдельный вид, и увидеть его в тестах нельзя.

        Проверяем хотя бы то, что правила на месте: без них поворот оставил бы
        обычное приложение, растянутое поперёк экрана.
        """
        css = anon.get("/static/style.css").text
        assert '[data-mode="deck"]' in css
        assert ".deck-side" in css
