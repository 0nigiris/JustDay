...
# Usage: setup-hotkey.sh [--talk "Meta+J"] [--extra F19] [--cancel "Meta+Shift+J"] [--mouse ExtraButton1] [--remove]
#
...
set -euo pipefail
TALK="Meta+J"; CANCEL="Meta+Shift+J"; EXTRA="F19"; MOUSE=""; REMOVE=0
while (($#)); do
  case "$1" in
    --talk) TALK="$2"; shift ;;
...
if ((REMOVE)); then
  for id in net.local.justday.desktop net.local.justday-stop.desktop; do
    unregister "$id"
