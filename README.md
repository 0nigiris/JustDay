...
Личная система удалённого управления домашними ПК с телефона: состояние,
пробуждение, выключение, запуск приложений, терминал. Плюс аппаратный
контроллер на ESP32 для того, что нельзя сделать программно.
...
| Сторож основного ПК в прошивке: сам будит его при пропаже | настроен на плате, **побудка на железе не проверена** |
| Прошивка ESP32-S3 (без PSRAM) | залита в плату, консоль и NVS проверены |
| Переключение KVM | абстракция готова, железа нет |

194 автоматических теста, `ruff` и `mypy --strict` без замечаний.
...
agent_token_env = "REMO32_AGENT_TOKEN_VASYA"
...
```
REMO32_AGENT_TOKEN_VASYA=токен-с-первого-ПК
REMO32_AGENT_TOKEN_MINEVPSEX=токен-со-второго-ПК
```
...
2. Получить сертификат:
   ...
   ```bash
   tailscale cert vasya.tailnet-abc123.ts.net
   ```
...
Отредактируйте `~/.config/remo32/agent.toml` и перезапустите агент.
